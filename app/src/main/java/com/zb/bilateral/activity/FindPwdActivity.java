...
public class FindPwdActivity extends BaseNewActivity<RegisterPresenter> implements RegisterView {
    ...
    public void register(String phone,String code ,String pwd) {
        if (AppUtil.checkNetWork(mCctivity)) {
            mvpPresenter.validateCode(phone,code,pwd);
        } else {
            AppToast.makeToast(mCctivity, "网络异常");
        }
    }
    ...
    public void SendSuccess(CommitModel commitModel) {
        AppToast.makeToast(mCctivity,"找回密码成功");
        finish();
    }
