...
import com.zb.bilateral.adapter.DynamicListAdapter;
...
public class DynamicListActivity extends LazyFragment<PublicPresenter<ActivityListModel>> implements SwipeRefreshLayout.OnRefreshListener,PublicView<ActivityListModel>{
    ...
    public static DynamicListActivity newInstance(String category, int multiType,String museum_id) {
        DynamicListActivity maintenanceFragment = new DynamicListActivity();
    ...
    private void initRecyclerView() {
        ...
        dynamicAdapter = new DynamicListAdapter(mContext);
        pLuRecyclerViewAdapter = new LuRecyclerViewAdapter(dynamicAdapter);
        ...
        pLuRecyclerViewAdapter.setOnItemClickListener(new com.github.jdsjlzx.interfaces.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                intent.putExtra("activity_id",dynamicAdapter.getDataList().get(position).getId());
                intent.putExtra("status",dynamicAdapter.getDataList().get(position).getStatus());
    ...
    public void acts(boolean is_load) {
        ...
        if (AppUtil.checkNetWork(mContext)) {
            mvpPresenter.acts(museumId,""+pageNum,token,is_load);
        } else {
    ...
    public void SendResultSuccess(ActivityListModel activityListModel) {
        ...
        dynamicAdapter.addAll(activityListModel.getActList());
